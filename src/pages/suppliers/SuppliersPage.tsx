...
const SUPPLIER_CATEGORIES = [
  'Pavers & Hardscape', 'Stone & Countertop', 'Sand & Base Materials',
  'Sealants & Chemicals', 'Equipment & Tools', 'Subcontractor', 'Other',
]

const catColors: Record<string, string> = {
  'Pavers & Hardscape': 'orange', 'Stone & Countertop': 'blue', 'Sand & Base Materials': 'yellow',
  'Sealants & Chemicals': 'green', 'Equipment & Tools': 'purple', 'Subcontractor': 'gray', 'Other': 'gray',
}

const emptyForm = { name: '', category: '', contact_name: '', email: '', phone: '', address: '', notes: '' }
...
export default function SuppliersPage() {
  ...
  function openEdit(supplier: Supplier) {
    setEditing(supplier)
    setForm({ name: supplier.name, category: supplier.category ?? '', contact_name: supplier.contact_name ?? '', email: supplier.email ?? '', phone: supplier.phone ?? '', address: supplier.address ?? '', notes: supplier.notes ?? '' })
    setModalOpen(true)
  }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const payload = { ...form, category: form.category || null, contact_name: form.contact_name || null, email: form.email || null, phone: form.phone || null, address: form.address || null, notes: form.notes || null }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-[18px] font-bold text-navy-900">Suppliers / Vendors</h1>
      ...
      <Card>
        <div className="border-b border-stone-100 px-4 py-3">
          <div className="relative max-w-sm">
            ...
            <input className="w-full rounded-[10px] border border-stone-200 py-2 pl-10 pr-3 text-[13px] placeholder:text-stone-400 focus:border-navy-900 focus:outline-none focus:ring-[3px] focus:ring-navy-900/[0.08]" placeholder="Search suppliers..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Supplier' : 'New Supplier'}>
        <div className="space-y-4">
          ...
          <Select label="Category" id="sup-cat" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} options={SUPPLIER_CATEGORIES.map(c => ({ value: c, label: c }))} />
