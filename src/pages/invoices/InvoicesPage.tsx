...
const emptyForm = { client_id: '', job_id: '', status: 'Draft' as InvoiceStatus, line_items: [{ ...emptyLine }], tax_rate: 7, notes: '', due_date: '' }

export default function InvoicesPage() {
  const { data: invoices, loading, insert, update, remove, fetch: refetch } = useTable<Invoice>('invoices')
  ...
  const filtered = invoices.filter(e => (clientMap[e.client_id] ?? '').toLowerCase().includes(search.toLowerCase()) || e.invoice_number.toLowerCase().includes(search.toLowerCase()))
  ...
  function openPreview(inv: Invoice) { setPreviewInvoice(inv); setPreviewOpen(true) }
  ...
  async function handleSave() {
    ...
    const payload = {
      invoice_number: editing?.invoice_number ?? `INV-${String(count).padStart(4, '0')}`,
      client_id: form.client_id, job_id: form.job_id || null, estimate_id: null, status: form.status,
  ...
  async function markPaid(inv: Invoice) {
    await update(inv.id, { status: 'Paid' as InvoiceStatus, paid_date: new Date().toISOString().split('T')[0] })
    refetch()
  }

  async function handleDelete() {
    if (editing && confirm('Delete this invoice?')) { await remove(editing.id); setModalOpen(false) }
  }
    ...
    <div className="space-y-6 p-6">
        ...
        {loading ? <p className="p-6 text-sm text-stone-500">Loading...</p> : (
          <Table
            ...
            columns={[
              ...
              { key: 'client', header: 'Client', render: i => clientMap[i.client_id] ?? '-' },
              ...
              { key: 'actions', header: '', render: i => (
                <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                  {i.status !== 'Paid' && (
                    <Button variant="ghost" size="sm" onClick={() => markPaid(i)} title="Mark as Paid">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                    </Button>
                  )}
                  <Button variant="ghost" size="sm" onClick={() => openPreview(i)}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Invoice' : 'New Invoice'} wide>
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {form.line_items.map((line, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-end">
                  ...
                  <div className="col-span-2"><input className="w-full rounded-lg border border-stone-300 px-2 py-1.5 text-sm" type="number" placeholder="Price" value={line.unit_price} onChange={e => updateLine(i, 'unit_price', Number(e.target.value))} /></div>
                  <div className="col-span-1 text-right text-sm font-medium">${(line.qty * line.unit_price).toFixed(2)}</div>
                  <div className="col-span-1">{form.line_items.length > 1 && <button onClick={() => removeLine(i)} className="text-red-500 text-sm hover:underline">X</button>}</div>
                </div>
              ))}
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Input label="Tax Rate (%)" id="inv-tax" type="number" value={form.tax_rate} onChange={e => setForm(f => ({ ...f, tax_rate: Number(e.target.value) }))} />
            ...
            <div ref={printRef} className="space-y-6 text-sm">
              <div className="flex justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="font-mono">{previewInvoice.invoice_number}</p>
                  <p className="text-stone-500">Date: {previewInvoice.created_at?.split('T')[0]}</p>
                  {previewInvoice.due_date && <p className="text-stone-500">Due: {previewInvoice.due_date}</p>}
                  <Badge color={statusColor(previewInvoice.status)}>{previewInvoice.status}</Badge>
                </div>
              </div>

              <div><p className="font-semibold">Bill To:</p><p>{clientMap[previewInvoice.client_id] ?? 'N/A'}</p></div>

              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-stone-300"><th className="py-2 text-left">Description</th><th className="py-2 text-right">Qty</th><th className="py-2 text-right">Unit</th><th className="py-2 text-right">Unit Price</th><th className="py-2 text-right">Amount</th></tr></thead>
                <tbody>
                  {(previewInvoice.line_items as InvoiceLineItem[]).map((item, i) => (
                    <tr key={i} className="border-b border-stone-100">
                      <td className="py-2">{item.description}</td><td className="py-2 text-right">{item.qty}</td><td className="py-2 text-right">{item.unit}</td><td className="py-2 text-right">${item.unit_price.toFixed(2)}</td><td className="py-2 text-right">${(item.qty * item.unit_price).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="ml-auto w-64 space-y-1 text-right">
                ...
                <div className="flex justify-between"><span>Tax ({previewInvoice.tax_rate}%)</span><span>${previewInvoice.tax_amount.toFixed(2)}</span></div>
                <div className="flex justify-between border-t border-stone-300 pt-1 text-base font-bold"><span>Total</span><span>${previewInvoice.total.toFixed(2)}</span></div>
              </div>

              {previewInvoice.notes && <div><p className="font-semibold">Notes:</p><p className="text-stone-600">{previewInvoice.notes}</p></div>}
              <div className="mt-8 border-t border-stone-200 pt-4"><p className="text-xs text-stone-400">Authorized by: {COMPANY.signatory} | {COMPANY.brand}</p></div>
            </div>
          </>
        )}
      </Modal>
